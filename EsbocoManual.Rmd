...
O CSV é um dos formatos de arquivos mais utilizados. Nesete formato o separador normalmente é uma vírgula ',' ou ponto e vírgula ';'. Ele pode ter um cabeçalho com os nomes das colunas ou não.  
```{r, eval=FALSE}
dataframe <- read.csv("Caminho-do-seu-arquivo.csv", header=TRUE)
head(dataframe) #Mostra as 6 primeiras linhas do seu arquivo, incluindo o cabeçalho
```
...
exl <- read_excel("Caminho-do-seu-arquivo.xlsx", 
                 sheet = "nome-da-aba", col_types = c("date", "text", "text", 
...
Basta qualquer valor NA (Not Available), NaN ( Not a Number) ou NULL (sem valor, diferente de NA pu NaN) para que o R não faça nehuma operação e retorne um erro. ~~O que por um lado pode ser uma grande dor de cabeça, por outro, o aviso é válido servindo de alerta para que tratemos esses valores.~~  
...
NA's podem ser oriundos de erro de digitação, perguntas não respondidas em pesquisas, ~~"joins" mal executados~~ e por aí vai.  

Dito isso, vou listar algumas opções para tratamentos de NA's, e cabe a você decidir o que vai fazer eles.  



Inicialmente, vamos criar um data frame pequeninho, com alguns NA's, só para testar as funções.
...
Com uma abordagem mais avançada, poderíamos também prever o valor dos NA, Nan ou Null e substituí-los por essa estimativa.  
```{r, warning=FALSE}
library(dplyr)
